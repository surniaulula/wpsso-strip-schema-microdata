	...
	class WpssoSsm {
		...
		public static function show_required_notices() {
			...
			foreach ( $info[ 'req' ] as $ext => $req_info ) {
				...
				$notice_msg = __( 'The %1$s add-on requires the %2$s plugin &mdash; install and activate the plugin or <a href="%3$s">deactivate this add-on</a>.', 'wpsso-strip-schema-microdata' );
		...
		private function min_version_notice() {
			...
			if ( is_admin() ) {
				...
				$notice_msg = sprintf( __( 'The %1$s version %2$s add-on requires %3$s version %4$s or newer (version %5$s is currently installed).',
					'wpsso-strip-schema-microdata' ), $info[ 'name' ], $info[ 'version' ], $req_info[ 'name' ], $req_info[ 'min_version' ],
