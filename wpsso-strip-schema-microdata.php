...
if ( ! class_exists( 'WpssoSsm' ) ) {
	...
	class WpssoSsm {
		 ...
		 */
		private static $ext           = 'wpssossm';
		private static $p_ext         = 'ssm';
		...
		private static function get_missing_requirements() {
			...
			$info = WpssoSsmConfig::$cf[ 'plugin' ][ self::$ext ];
			...
			foreach ( $info[ 'req' ] as $key => $req_info ) {
				...
				if ( ! empty( $req_info[ 'version_global' ] ) && ! empty( $GLOBALS[ $req_info[ 'version_global' ] ] ) ) {
					...
					$notice_msg = __( 'The %1$s version %2$s add-on requires the %3$s plugin &mdash; please activate the missing plugin.', 'wpsso-strip-schema-microdata' );
				...
				if ( ! empty( $req_info[ 'version' ] ) ) {
					...
					if ( ! empty( $req_info[ 'min_version' ] ) ) {
						...
						if ( version_compare( $req_info[ 'version' ], $req_info[ 'min_version' ], '<' ) ) {
							...
							$notice_msg = __( 'The %1$s version %2$s add-on requires %3$s version %4$s or newer (version %5$s is currently installed).', 'wpsso-strip-schema-microdata' );
